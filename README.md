...
- **93 道标准测试题**：基于 MBTI M 版本编制，题目顺序随机打乱，确保测试结果客观。
...
1. 下载或克隆本仓库。
2. 将 `image` 文件夹放入根目录，并确保包含 16 张对应人格类型的图片（文件名格式：`类型-typies.jpg`，例如 `INTJ-typies.jpg`）。
3. 使用任意 HTTP 服务器打开 `index.html`（推荐使用 Live Server 等工具，避免本地文件跨域问题）。
4. 开始测试：点击「开始测试」→ 阅读须知 → 逐题作答 → 获得结果并查看详细分析。
...
| 类别 | 说明 |
|------|------|
| 结构 / 样式 | HTML5 + CSS3（Flexbox / Grid 布局） |
| 逻辑 | 原生 JavaScript（ES6） |
| 数据存储 | LocalStorage（进度存储） |
| 依赖 | 无第三方依赖，纯原生实现 |
...
本项目采用 [MIT License](https://opensource.org/licenses/MIT)。可自由使用、修改、分发，但请保留原作者声明。
